...
GLuint make_module(const char* filepath, const unsigned int module_type) {
    ...
    const GLuint shader_module = glCreateShader(module_type);
...
GLuint make_shader(const char* vertex_filepath, const char* fragment_filepath) {
    const GLuint vertex_module = make_module(vertex_filepath, GL_VERTEX_SHADER);
    const GLuint fragment_module =
