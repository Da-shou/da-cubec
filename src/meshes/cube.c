...
void cube_init(cube_t* c) {
        ...
        if (c->mesh->vbos == NULL) {
                c->mesh->vbos = malloc(sizeof(unsigned int) * 2);
        }
	...
	if (c->mesh->vao == 0) glGenVertexArrays(1, &c->mesh->vao);
        ...
        glBufferData(GL_ARRAY_BUFFER, 72 * sizeof(float), CUBE_VERTICES,
                     GL_STATIC_DRAW);
        ...
        glBufferData(GL_ARRAY_BUFFER, 48 * sizeof(float),
                     CUBE_TEXTURE_COORDINATES, GL_STATIC_DRAW);
        ...
        glBufferData(GL_ELEMENT_ARRAY_BUFFER,
                     CUBE_INDEX_COUNT * sizeof(uint64_t), CUBE_INDICES,
                     GL_STATIC_DRAW);
}
