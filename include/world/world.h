 ...
 */
typedef void (*chunk_generator_t)(chunk_t* chunk, int world_cx, int world_cz, const void* userdata);
 ...
 */
int world_rebuild_after_change(world_t* world, int chunk_x, int chunk_z, int local_x, int local_z);
/**
...
/**
 * @brief Returns the block type at the given integer block coordinates.
 * @param world Pointer to the world.
 * @param block_x World Block X coordinate.
 * @param block_y World Block Y coordinate.
 * @param block_z World Block Z coordinate.
 * @return Block type, or BLOCK_AIR if out of bounds or chunk not loaded.
 */
uint8_t world_get_block(world_t* world, int block_x, int block_y, int block_z);
